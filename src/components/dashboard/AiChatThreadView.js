...
import {
  ...
  markAiChatThreadReadAction,
} from "@/app/dashboard-araf/aiChatActions";
import { ConfirmModal, StatusModal } from "@/components/dashboard/Modal";
import { formatAiChatMessageTime } from "@/lib/aiChatMessages";

export default function AiChatThreadView({ thread }) {
  ...
  useEffect(() => {
    ...
    const key = localThread.ipKey;
    const timer = window.setTimeout(() => {
      startTransition(async () => {
        const result = await markAiChatThreadReadAction(key);
        if (result?.success) {
          setLocalThread((prev) =>
            prev
              ? {
    ...
    return () => window.clearTimeout(timer);
  }, [localThread?.ipKey, localThread?.unreadCount, router]);
  ...
  const confirmDeleteThread = () => {
    ...
    startTransition(async () => {
      const result = await deleteAiChatThreadAction(key);
      if (result?.success) {
        setFlash({ type: "success", text: "Conversation deleted." });
        router.push("/dashboard-araf/ai-chats");
        router.refresh();
      } else {
        setFlash({
          type: "error",
    ...
    return (
      <div className="flex min-h-[360px] flex-col items-center justify-center rounded-xl bg-surface-container-lowest/90 px-6 py-12 text-center">
        ...
        <p className="mt-1 max-w-xs text-[12px] text-on-surface-variant">
          Pick a visitor from the AI Chat sidebar.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-hidden rounded-xl bg-surface-container-lowest/90">
      <div className="flex min-h-[480px] flex-col">
        <header className="flex h-11 items-center gap-2 border-b border-border/60 px-3.5 sm:px-4">
          <div className="min-w-0 flex-1">
            <p className="truncate font-label-mono text-[13px] font-medium text-on-surface">
              {localThread.name || localThread.ip}
            </p>
            <p className="truncate text-[11px] text-on-surface-variant">
              {localThread.messageCount} question
          ...
          {localThread.messages.map((msg) => (
            ...
            >
              <div className="mb-1 flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                <span className="font-label-mono text-[12px] font-medium text-on-surface">
                  {msg.ip}
                </span>
                <time className="text-[10px] text-on-surface-variant">
                  {formatAiChatMessageTime(msg.createdAt)}
                </time>
              </div>
      ...
      <ConfirmModal
        open={confirmDelete}
        title="Delete conversation?"
        message={`Remove all AI questions from ${localThread.ip}? This cannot be undone.`}
        confirmLabel="Delete"
