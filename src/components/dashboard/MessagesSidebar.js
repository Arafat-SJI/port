...
import { messageThreadHref } from "@/data/dashboard";
...
export default function MessagesSidebar({ threads = [], onNavigate, loadError }) {
    ...
    <aside className="flex h-full w-[220px] shrink-0 flex-col border-r border-border bg-surface-container-lowest">
      <div className="flex h-11 items-center gap-1 px-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-[12px] font-medium text-on-surface">Message</p>
        </div>
      ...
      <nav className="min-h-0 flex-1 overflow-y-auto custom-scrollbar pt-2 pb-3">
        ...
        {loadError ? (
          <p className="px-3 py-2 text-[11px] leading-relaxed text-error">
            Could not load senders. Run migration 016.
          </p>
        ) : null}

        {!loadError && list.length === 0 ? (
          <p className="px-3 py-2 text-[11px] leading-relaxed text-on-surface-variant">
            No messages yet. Contact form submissions appear here by email.
          </p>
        ) : null}

        {list.map((thread) => {
          const href = messageThreadHref(thread.emailKey);
          const active =
            pathname === href ||
            decodeURIComponent(pathname.split("/").pop() || "") === thread.emailKey;
          return (
            <Link
              key={thread.emailKey}
              href={href}
