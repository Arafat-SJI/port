...
import { saveClubingContentAction } from "@/app/dashboard-araf/clubingActions";
...
export default function ClubingEditor({ initialContent }) {
  const [state, formAction, pending] = useActionState(
    saveClubingContentAction,
    initialState
  );
  const [content, setContent] = useState(() =>
    normalizeClubingContent(initialContent)
  );
  const [savedSnapshot, setSavedSnapshot] = useState(() =>
    JSON.stringify(normalizeClubingContent(initialContent))
  );
  ...
  useEffect(() => {
    if (state?.success && state.content) {
      const next = normalizeClubingContent(state.content);
      setContent(next);
  ...
  const addItem = () => {
    setContent((prev) => ({
      ...prev,
      items: [...prev.items, createEmptyClubingItem()],
    }));
  };
    ...
    <div className="space-y-6">
      <form action={formAction} className="space-y-5">
        ...
        <section className="space-y-3 rounded-xl bg-surface-container-lowest/90 p-4 sm:p-5">
          ...
          <div>
            <label className={labelClass} htmlFor="clubing-title">
              Header title
            </label>
            <input
              id="clubing-title"
        ...
        {content.items.map((item, index) => (
          ...
          >
            ...
            <div>
              <label className={labelClass} htmlFor={`name-${item.id}`}>
                Name
              </label>
              <input
                id={`name-${item.id}`}
                ...
                className={fieldClass}
                placeholder="Buet Programming Club"
                required
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Short description of the club role…"
              />
            </div>
          </section>
        ))}
