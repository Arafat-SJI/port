...
import { saveProjectsContentAction } from "@/app/dashboard-araf/projectsActions";
...
export default function ProjectsEditor({ initialContent }) {
  const [state, formAction, pending] = useActionState(
    saveProjectsContentAction,
    initialState
  );
  const [content, setContent] = useState(() =>
    normalizeProjectsContent(initialContent)
  );
  const [savedSnapshot, setSavedSnapshot] = useState(() =>
    JSON.stringify(normalizeProjectsContent(initialContent))
  );
  ...
  useEffect(() => {
    if (state?.success && state.content) {
      const next = normalizeProjectsContent(state.content);
      setContent(next);
  ...
  const addItem = () => {
    setContent((prev) => ({
      ...prev,
      items: [...prev.items, createEmptyProjectsItem()],
    }));
  };
        ...
        <section className="space-y-3 rounded-xl bg-surface-container-lowest/90 p-4 sm:p-5">
          ...
          <div>
            ...
            <input
              ...
              className={fieldClass}
              placeholder="Tools and platforms engineered for scale."
            />
          </div>
        </section>
